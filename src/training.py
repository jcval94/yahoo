    ...
    for ticker, df in data.items():
        ...
        with timed_stage(f"train RF {ticker}"):
            try:
                ...
                rf_path = MODEL_DIR / f"{ticker}_{frequency}_rf.b64"
                save_model_text(rf, rf_path)
        ...
        with timed_stage(f"train XGB {ticker}"):
            try:
                ...
                xgb_path = MODEL_DIR / f"{ticker}_{frequency}_xgb.b64"
                save_model_text(xgb, xgb_path)
        ...
        with timed_stage(f"train LSTM {ticker}"):
            try:
                ...
                lstm_path = MODEL_DIR / f"{ticker}_{frequency}_lstm.b64"
                save_model_text(lstm, lstm_path)
