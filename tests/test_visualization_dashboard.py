...
def test_prepare_action_recommendations_includes_lstm_column_without_lstm_preds(monkeypatch, tmp_path):
    run_date = pd.Timestamp("2026-03-11")
    preds = pd.DataFrame([
        {
            "ticker": "AAA",
    ...
    assert out is not None
    assert not out.empty
    assert "pred_LSTM" in out.columns
    assert out.iloc[0]["pred_LSTM"] == "-"


def test_prepare_action_recommendations_maps_sequential_to_lstm(monkeypatch, tmp_path):
    run_date = pd.Timestamp("2026-03-11")
    preds = pd.DataFrame([
        {
            "ticker": "AAA",
    ...
    def _fake_eval(*args, **kwargs):
        return {"ticker": "AAA", "best_model": "LSTM", "score": -4.0, "actual": 100.0}
    ...
    assert not out.empty
    assert "pred_LSTM" in out.columns
    assert out.iloc[0]["pred_LSTM"] == "BAJA"
